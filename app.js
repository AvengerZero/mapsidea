...
app.get("/markers", async function(req, res) {
  let menu;
  try {
    menu = await queryHome("SELECT * FROM markers;");
    res.json(menu);
  } catch (err) {
    res.type("text");
    res.status(FILE_ERROR).send(SERVER_ERROR);
...
app.get("/markers/:path", async function(req, res) {
  let menu;
  try {
    menu = await queryHome("SELECT * FROM markers WHERE path = " +
                          req.params.path +
                          " ORDER BY spot ASC;");
    res.json(menu);
  } catch (err) {
...
app.get("/newNumRoute", async function(req, res) {
  let db;
  try {
    db = await getDB();
    let newNum = await db.query("SELECT MAX(path) FROM `markers`;");
    db.end();
    res.json(newNum[0]["MAX(path)"]);
  } catch (err) {
    res.type("text");
    res.status(FILE_ERROR).send(SERVER_ERROR);
  }
...
app.post("/addroute", async function(req, res) {
  ...
  try {
    db = await getDB();
    let checkingbid =
      await db.query("INSERT INTO `markers` (`path`, `address`, `spot`) VALUES"
      + "("+ data.routeId  + ", \"" + data.location + "\", "+ data.routeNum + ")");
    if (checkingbid[0]) {
      res.send("PLEASE REFRESH PAGE, NEW DATA");
    }
    db.end();
    res.send("" + data.routeId);
  }catch (err) {
    res.type("text");
    console.log(err);
    res.status(FILE_ERROR).send(SERVER_ERROR);
...
app.post("/addroutedescription", async function(req, res) {
  let data = req.body;
  let db;
  res.type("text");
  console.log(req.body);
  try {
    db = await getDB();
    let checkingbid =
      await db.query("INSERT INTO `route_data` (`id`, `route_title`, `description`, `tags`, `author`)"
       + "VALUES (" + data.id +
       ", \"" + data.route_title + "\"" +
       ", \"" + data.description + "\"" +
       ", \"" + data.tags + "\"" +
...
function verifyRoute(req, res, next) {
  if (req.body.location && req.body.stopover && req.body.routeNum) {
    next();
  } else {
    res.type("text");
